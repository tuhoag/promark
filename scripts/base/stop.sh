...
function stopNetwork() {
    ...
    local peersServicesPath="-f ${DOCKER_COMPOSE_DIR_PATH}/peers-${orgNum}-${peerNum}.yml"
    FABRIC_LOG=$logLevel COMPOSE_PROJECT_NAME=$PROJECT_NAME PROJECT_NAME=$PROJECT_NAME IMAGE_TAG=$FABRIC_VERSION NUM_ORGS=$orgNum NUM_PEERS=$peerNum docker-compose $coreServicesPath $peersServicesPath down -v 2>&1
    { set +x; } 2>/dev/null
