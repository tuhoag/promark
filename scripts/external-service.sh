...
function runService() {
    ...
    if test $mode -eq 0; then
        ...
        FABRIC_LOG=$log_level COMPOSE_PROJECT_NAME=$PROJECT_NAME PROJECT_NAME=$PROJECT_NAME IMAGE_TAG=$FABRIC_VERSION docker-compose -f ${docker_compose_path} run --service-ports peer0.pub0.promark.com /bin/sh 2>&1
